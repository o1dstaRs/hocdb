...
function runBasicTest() {
    ...
    try {
        ...
        db = new HOCDB(TICKER, DATA_DIR, schema as any);
        ...
        if (data.length > 0) {
            ...
            if (stats.count !== 1000n) throw new Error("Stats count mismatch");

            const latest = db.getLatest("usd");
            console.log(`Latest 'usd': val=${latest.value}, ts=${latest.timestamp}`);
            if (latest.timestamp !== 999999n) throw new Error("Latest timestamp mismatch");
        }
...
function runRingBufferTest() {
    ...
    let dbRing;
    try {
        dbRing = new HOCDB(RING_TICKER, RING_DATA_DIR, schema as any, {
            max_file_size: HOCDB.headerSize() + 2 * 24, // 64-byte header + 2 records of 24 bytes
...
function runFlushTest() {
    ...
    let db;
    try {
        db = new HOCDB("TEST_FLUSH", testDir, schema, {
...
function runFilteringTest() {
    ...
    let db;
    try {
        db = new HOCDB("FILTER_TEST", testDir, [
            { name: "timestamp", type: "i64" },
            { name: "price", type: "f64" },
            { name: "event", type: "i64" }
        ], { overwrite_on_full: true });
